...
type PreviewStatus = PreviewEventItem["status"];

const dotColor: Record<PreviewStatus, string> = {
  info: "bg-text-muted",
...
export const PipelinePreview: React.FC<PipelinePreviewProps> = ({ events, stack }) => {
  ...
  return (
    <section ref={ref} id="pipeline" className="border-b border-border py-28 md:py-40">
      <div className="max-w-6xl mx-auto px-6">
              ...
              <div className="bg-canvas p-6 flex flex-col justify-between">
                ...
                <div className="space-y-3 py-1">
                  {events.map((ev, i) => (
                    <div
                      key={i}
                      className={`terminal-row flex items-start gap-3 text-xs font-mono ${isInView ? "revealed" : ""}`}
        ...
        <div id="stack" className={`mt-14 pt-8 pb-16 scroll-mt-24 border-t border-border/60 reveal ${isInView ? "revealed" : ""}`}>
