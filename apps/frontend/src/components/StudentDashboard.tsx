                ...
                <section>
                    ...
                    <div className="space-y-6">
                        {recentSubmissions.filter(s => s.status === 'GRADED').slice(0, 3).map(sub => (
                            <div key={sub.id} className="bg-surface border-[4px] border-on-surface brutal-shadow flex flex-col hover:-translate-y-1 transition-transform duration-75">
                                ...
                                <div className="p-6 flex-1 flex flex-col justify-center items-center bg-secondary-fixed border-b-[4px] border-on-surface">
                                    ...
                                    <div className="font-label-mono text-label-mono font-bold bg-surface border-[2px] border-on-surface px-3 py-1 uppercase">
                                        {sub.score! >= 90 ? 'Excellent' : 'Graded'}
                                    </div>
                                </div>
                                <div className="p-4 bg-surface flex justify-between items-center">
                                    <span className="font-body-md text-body-md font-bold truncate pr-4">ID: {sub.id.substring(0, 6)}</span>
                                    <Button variant="brutal" size="icon" onClick={() => setSelectedSubmission(sub)} aria-label="View feedback">
                        ...
                        {recentSubmissions.filter(s => s.status === 'GRADED').length === 0 && (
                            <div className="border-[4px] border-on-surface border-dashed p-12 text-center">
                                <p className="font-headline-md text-4xl font-black uppercase tracking-tighter text-on-surface-variant">
                                    NO FEEDBACK<br />YET
                                </p>
